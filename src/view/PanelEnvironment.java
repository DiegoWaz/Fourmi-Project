...
public class PanelEnvironment extends JFrame implements ActionListener
{
	...
	/** constructeur par defaut, definit la couleur du terrain et lance l'initialisation*/
	public PanelEnvironment()
	{
		colTerrain = new Color(0, 80, 0); // vert fonce
		init();
	}
	
	/** constructeur par defaut, definit la couleur du terrain, relie ce panneau graphique au modele, lance l'initialisation, ainsi que le TImer*/
	public PanelEnvironment(Terrain _matrice, int tempo, View window)
	{
		terrain = _matrice;
		...
		for(int i=0; i<taille; i++)
			for(int j=0; j<taille; j++)
			{
				...
				if (cell.getNourriture()>0) // affichage des zones de nourritures 
				{
					...
					int b = (int)(ratio * 255); 
					g2d.setColor(new Color(0,0,b)); // la case est +- bleue en fonction de la densite de nourriture
				...
				if (cell.getPheromone()>0) // s'il y a une trace de pheromone
				{
					...
					float transparence = (float)ratio + 0.2f; if (transparence>1f) transparence = 1f; // calcul du degre de transparence
					...
					g2d.setColor(c);
					g2d.fillRect((i+1)*espace, (j+1)*espace, espace, espace);
					alpha = AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1f); //retablissement a un affichage opaque
