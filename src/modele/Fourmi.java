...
public class Fourmi {
	...
	/**dose de pheromone que depose la fourmi*/
	...
	public void evoluer()
	{
		...
		case CHERCHER: //recherche de nourriture
			direction = getBestDirection(); //s'orienter vers la case offrant le plus de nourriture, sinon le plus de pheromone, sinon au hasard devant
		...
		case REVENIR:
			...
			if (grille[x][y].isNid())  // si elle est dans le nid, elle passe a l'etat suivant
		...
		case DEPOSER:
			...
			bougerVersDirection();// et avance
			etat = EtatFourmi.CHERCHER; // elle passe a l'etat suivant
			break;
		case EVITER:
	 ...
	 * sinon le plus de pheromone, sinon retourne une des trois directions devant la fourmi
	 * pour un comportement "realiste" la fourmi a un rayon de braquage de 45e
	 * -> elle peut aller soit tout droit, e gauche devant ou e droite devant*/
	...
	/**donne la prochaine case dans la direction donnee
	 * @param dir la direction
	 * @return la cellule voisine dans la direction donnee, null si aucune cellule*/
